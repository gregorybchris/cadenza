...
class TestPitch:
    def test_add_basic(self) -> None:
        pitch_a = Pitch(note=Note.new_c(), octave=4)
        pitch_b = pitch_a.transpose_unsafe(1)
        assert pitch_b.note == Note.new_c_sharp()
        assert pitch_b.octave == 4

    def test_add_across_octaves(self) -> None:
        pitch_a = Pitch(note=Note.new_g(), octave=3)
        pitch_b = pitch_a.transpose_unsafe(6)
        assert pitch_b.note == Note.new_c_sharp()
        assert pitch_b.octave == 4

    @pytest.mark.skip(reason="Safe pitch transpose not yet implemented.")
    def test_add_with_flat(self) -> None:
        pitch_a = Pitch(note=Note.new_d_flat(), octave=4)
        pitch_b = pitch_a.transpose_unsafe(5)
        assert pitch_b.note == Note.new_g_flat()
        assert pitch_b.octave == 4

    def test_to_frequency_concert_a(self) -> None:
        middle_c = Pitch(note=Note.new_c(), octave=4)
        concert_a = middle_c.transpose_unsafe(9)
